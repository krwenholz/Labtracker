...
public class EventListener_Main extends Extension implements CompileListener {
    ...
    /**
     * Called on startup of BlueJ.  Registers as a listener for compile 
     * and class events.
     * @param blueJ the instance of BlueJ
     */
	...
	for(BProject i : blueJ.getOpenProjects()){
		...
		System.out.println("current - project specific: "
				   +(this.time-time));
	...
	this.actTrack = new ProgressTracking(this.blueJ, this.antennae, 
					     EventListener_Main.TRACKING_DELAY);
    ...
    /**
     * Called by BlueJ when the plugin is closed.  Basically just cuts off the 
     * database connection.
     */
	...
	public void compileError(CompileEvent compErr) {
	    ...
	    if(f.length>0){
		methodName = this.getSignature(f[0].getName(), 
					       compErr.getErrorLineNumber());
	    }
	    
	    this.antennae.emitEvent(EventListener_Main.COMPILE_ERROR, 
	        ""+methodName,"  Message: "+compErr.getErrorMessage());
	}
	...
	/*this.antennae.emitEvent("Compile", "Failure","ErrLine: "+
	compFail.getErrorLineNumber()+"  Message: "+
	...
	/*String methodName = "unknown";
	  ...
	  if(f.length>0){
	  methodName = this.getMethodSig(f[0].getName(), 
	   compWarn.getErrorLineNumber());
	  }
